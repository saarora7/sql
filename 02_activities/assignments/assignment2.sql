...
SELECT
  ...
  COUNT(*) OVER (PARTITION BY customer_id, product_id) AS times_product_purchased
FROM customer_purchases;
...
SELECT product_size, product_id, product_name
FROM product
...
SELECT 
    v.vendor_name,
    p.product_name,
    5 * COUNT(c.customer_id) * vi.original_price AS total_revenue
FROM (
